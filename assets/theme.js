...
class CollectionHover extends HTMLElement {
  ...
  onMouseEnter(event) {
    ...
    if (!this.imageVisible && this.imageHover) {
      ...
      setTimeout(() => {
        this.imageHover.style.transition = 'opacity 0.1s ease, transform 0.1s ease';
      }, 10);
  ...
  animateImagePosition() {
    ...
    const image = this.imageHover.querySelector('img');
    if (image) {
      const relX = (this.mousePosition.x - this.bounds.left) / this.bounds.width;
      const relY = (this.mousePosition.y - this.bounds.top) / this.bounds.height;
    ...
    this.animationId = requestAnimationFrame(this.animateImagePosition.bind(this));
  }
