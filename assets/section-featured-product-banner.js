    ...
    class MediaEffect extends HTMLElement {
      constructor() {
        ...
        this.elements = Array.from(
          this.querySelectorAll(".media-effect__item")
        );
        this.container =
          this.closest(".section-featured-product-banner") || this;
        this.bounds = null;
      ...
      applySpringAnimation() {
        ...
        this.elements.forEach((element, index) => {
          if (!element.dataset.factorRandomX) {
            element.dataset.factorRandomX = (0.7 + Math.random() * 0.6).toFixed(
              2
            );
            element.dataset.factorRandomY = (0.7 + Math.random() * 0.6).toFixed(
              2
            );
            element.dataset.offsetX = (Math.random() * 20 - 10).toFixed(1);
            element.dataset.offsetY = (Math.random() * 20 - 10).toFixed(1);
            element.dataset.angleOffset = (Math.random() * 0.4 - 0.2).toFixed(
              2
            );
          }
          ...
          const rotatedX =
            relativeX * Math.cos(angleOffset) -
            relativeY * Math.sin(angleOffset);
          const rotatedY =
            relativeX * Math.sin(angleOffset) +
            relativeY * Math.cos(angleOffset);

          const targetX =
            rotatedX * speedFactor * this.bounds.width * factorRandomX +
            offsetX;
          const targetY =
            rotatedY * speedFactor * this.bounds.height * factorRandomY +
            offsetY;
      ...
      triggerRandomMovement() {
        ...
        this.elements.forEach((element, index) => {
          if (!element.dataset.factorRandomX) {
            element.dataset.factorRandomX = (0.8 + Math.random() * 0.4).toFixed(
              2
            );
            element.dataset.factorRandomY = (0.8 + Math.random() * 0.4).toFixed(
              2
            );
            element.dataset.offsetX = (Math.random() * 10 - 5).toFixed(1);
            element.dataset.offsetY = (Math.random() * 10 - 5).toFixed(1);
            element.dataset.angleOffset = (Math.random() * 0.2 - 0.1).toFixed(
              2
            );
          }
