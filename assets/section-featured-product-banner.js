class MediaEffect extends HTMLElement {
  constructor() {
    ...
    this.bounds = null;
    this.speedFactors = [0.02, 0.04, 0.06, 0.08];
  ...
  initialize() {
    ...
    if (typeof Motion !== 'undefined' && typeof Motion.motionValue === 'function') {
  ...
  scheduleSpringAnimation() {
    ...
    if (typeof Motion !== 'undefined' && Motion.frame) {
      ...
      this.animationId = requestAnimationFrame(() => this.applySpringAnimation());
    }
  }
  ...
  applySpringAnimation() {
    ...
    this.elements.forEach((element, index) => {
      ...
      const speedFactor = parseFloat(element.dataset.speedFactor) || this.speedFactors[index % this.speedFactors.length];
      ...
      const rotatedX = relativeX * Math.cos(angleOffset) - relativeY * Math.sin(angleOffset);
      const rotatedY = relativeX * Math.sin(angleOffset) + relativeY * Math.cos(angleOffset);

      const targetX = rotatedX * speedFactor * this.bounds.width * factorRandomX + offsetX;
      const targetY = rotatedY * speedFactor * this.bounds.height * factorRandomY + offsetY;
  ...
  triggerRandomMovement() {
    ...
    this.elements.forEach((element, index) => {
      ...
      const rotatedX = randomX * Math.cos(angleOffset) - randomY * Math.sin(angleOffset);
      const rotatedY = randomX * Math.sin(angleOffset) + randomY * Math.cos(angleOffset);
  ...
  setupElementStyles() {
    this.elements.forEach((element, index) => {
      ...
      element.dataset.speedFactor = this.speedFactors[index % this.speedFactors.length];
...
class ContentEffect extends HTMLElement {
  ...
  setupInitialStates() {
    this.infoElements.forEach((element) => {
      element.style.opacity = '0';
      if (typeof Motion === 'undefined') {
        element.style.transition = 'opacity 0.8s ease-out, transform 0.8s cubic-bezier(0.17, 0.67, 0.3, 1.33)';
      }
    });
  }
